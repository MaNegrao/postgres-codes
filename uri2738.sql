select name, trunc((score.math * 0.2)+(score.specific * 0.3)+(score.project_plan * 0.5), 2) as "media" from candidate inner join score on candidate.id = score.candidate_id order by media desc;
